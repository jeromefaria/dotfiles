...
| Gmail app-specific password | Revoked at https://myaccount.google.com/apppasswords and replaced; macOS Keychain entry updated | 2026-06-22 | Exposure was via a verification step — running `mail/scripts/get-gmail-pass.sh` printed the password to stdout (intended for piping into mbsync) and the value entered the conversation buffer. Not a git-history leak. Replacement verified by `mail/scripts/sync-mail.sh` end-to-end. The script's print-to-stdout behaviour is intentional, but it does mean any future verification of the keychain entry should use `… \| wc -c` rather than re-running the script raw. |
